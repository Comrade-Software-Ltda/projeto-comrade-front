...
export abstract class SystemPermissionRepository {
  abstract getSystemPermissionById(
    id: string
  ): Observable<SingleResultModel<SystemPermissionModel>>;
  abstract getAllSystemPermission(
    filter: PageFilterModel
  ): Observable<PageResultModel<SystemPermissionModel>>;
